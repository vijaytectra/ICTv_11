...
print("==========================================================================", flush=True)
print("             STARTING COMPREHENSIVE ICT BACKTEST AUDIT SUITE               ", flush=True)
print("==========================================================================", flush=True)
...
for pair in pairs:
    ...
    print(f"  {pair}: Resampled 5m candles={len(df_res)}, Signals={len(signals)}, Trades Executed={res['total_trades']}, Win Rate={res['win_rate_pct']}%, Net Profit=${res['net_profit']:.2f}", flush=True)

print(f"\nTotal Executed Portfolio Trades across 6 Pairs: {len(all_trades)}", flush=True)
...
def wilson_ci(k, n):
    ...
    hw = z * math.sqrt((p * (1 - p) + z**2 / (4 * n)) / n) / denom
...
print("\n--- WIN RATE AUDIT TABLE ---", flush=True)
print(pd.DataFrame(setup_rows).to_string(index=False), flush=True)
print(f"Sum of Setup Trades: {sum_setup_trades}", flush=True)
...
print(f"\nIntrabar Ambiguous Trades (both SL and TP hit in same exit candle): {ambig_cnt} ({ambig_cnt/max(1, len(all_trades))*100:.2f}%)", flush=True)
...
print("\n--- CHRONOLOGICAL OUT-OF-SAMPLE RESULTS ---", flush=True)
print(pd.DataFrame([
...
print("\n--- MONTE CARLO 10,000 SIMULATIONS ---", flush=True)
print(f"  5th Percentile Ending Balance:  ${np.percentile(final_bals, 5):,.2f}", flush=True)
print(f"  50th Percentile Ending Balance: ${np.percentile(final_bals, 50):,.2f}", flush=True)
print(f"  95th Percentile Ending Balance: ${np.percentile(final_bals, 95):,.2f}", flush=True)
print(f"  Median Max Drawdown:           {np.median(max_dds):.2f}%", flush=True)
print(f"  95th Percentile Max Drawdown:  {np.percentile(max_dds, 95):.2f}%", flush=True)

# 7. Cost Sensitivity
print("\n--- COST SENSITIVITY AUDIT (EURUSD) ---", flush=True)
...
print("\n==========================================================================", flush=True)
print("                    AUDIT SUITE COMPLETE                                  ", flush=True)
print("==========================================================================", flush=True)
