...
def load_pair_data(data_dir: str, pair: str, sample_ratio: float = 1.0) -> pd.DataFrame:
    ...
    and returns a clean, indexed DataFrame with open, high, low, close, volume, and spread_pips.
    """
    ...
    bid_df = pd.concat(bid_dfs, ignore_index=True).sort_values('time').drop_duplicates('time').reset_index(drop=True)
    
    # Process Ask data if available to calculate dynamic spread
    spread_pips = 1.0  # default fallback spread
