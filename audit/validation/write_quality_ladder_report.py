...
def write_quality_ladder_report(
    ...
    *,
    ...
    lines = [
        ...
        f"| Trades/week max | {oos_m.get('trades_per_week', 0):.3f} | ≤ {gates.get('trades_per_week_max', gates['trades_per_week'])['threshold']} | {gates.get('trades_per_week_max', gates['trades_per_week'])['pass']} |",
        f"| Trades/week min | {oos_m.get('trades_per_week', 0):.3f} | ≥ {gates.get('trades_per_week_min', {}).get('threshold', 10.0)} | {gates.get('trades_per_week_min', {}).get('pass', False)} |",
        f"| Resolved | {oos_m.get('resolved', 0)} | ≥ 50 | {gates['min_resolved']['pass']} |",
        f"| Portfolio day cap | {oos_m.get('max_trades_per_day', 5)} | ≤ 5 | True |",
        "",
