...
out_txt = r"C:\personal\ICT_v11\audit\validation\clean_validation_results.txt"
with open(out_txt, "w", encoding="utf-8") as out:
    ...
    p("==========================================================================")
    p("       CLEAN RE-VALIDATION ENGINE — UNBIASED ICT FOREX AUDIT               ")
    p("==========================================================================")
    p(f"Strategy Config Hash: {config_hash}")
    p(f"Pairs Monitored:       {', '.join(pairs)}")
    p(f"Starting Capital:     ${frozen_cfg['starting_capital']:.2f}")
    p(f"Risk per Trade:       {frozen_cfg['risk_percent']}% (${frozen_cfg['starting_capital'] * (frozen_cfg['risk_percent']/100):.2f})")
    p(f"Breakeven Rule:       +1.0R Trigger -> +0.2 pips lock")
    p("==========================================================================")
    ...
    for pair in pairs:
        p(f"[+] Loading 1m data & computing causal indicators for {pair}...")
        df_raw = load_pair_data(data_dir, pair, sample_ratio=0.1) # 0.1 ratio for fast audit
    ...
    p(f"Executed Portfolio Trades:   {len(all_trades)}")
    ...
    p(df_setup_table.to_string(index=False))
    ...
    p(df_splits.to_string(index=False))
    ...
    p(f"  95th % Max Consecutive Losses:  {np.percentile(max_consec_l, 95):.0f} Losses")

    hash1 = hashlib.sha256(open(trade_log_path, "rb").read()).hexdigest()
    p(f"\nTrade Log SHA-256 Hash (Run 1): {hash1}")

    p("\n==========================================================================")
    p("                   CLEAN RE-VALIDATION COMPLETE                           ")
    p("==========================================================================")
